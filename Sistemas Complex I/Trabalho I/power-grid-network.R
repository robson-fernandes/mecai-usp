...
plot_degree_distribution(G, FALSE,"Rede Power Grid - Distribuição do Grau") 
plot_degree_distribution(G, TRUE, "Rede Power Grid - Distribuição Acumulada do Grau") #Acumulada
...
menorCaminho = shortest_paths(G, v=V(G), to=V(G))
...
plot(h, 
     ...
     main = "Rede - Power Grid - Histograma - Distribuição Acumulada") 
...
hist(menorCaminho, 
     ...
     main = "Rede - Power Grid - Histograma")
...
hist(bt,
     ...
     main = "Rede - Power Grid - Distribuição de Probabilidade")
...
lines(density(eg, na.rm = T, from =0, 
              to = max(eg, na.rm = T)),col="red")
