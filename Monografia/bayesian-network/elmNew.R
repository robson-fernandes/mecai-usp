...
fitModelBayesGLM <- function(training.set)
{
  ...
  fit.bayesian.glm <- elmtrain(venda ~ 
                                   mes +
                                   grupoBebidas +
                                   grupoCarne  +
                                   grupoUniforme +
...
{
  predict.bayesian.glm <- predict(fit.bayesian.glm,
...
plotBGLM <- function(ds.resultado)
{
  ...
  p <- plot_ly(as.data.frame(ds.resultado), 
    ...
    add_trace(y = ~previsto,
              line = list(shape = "spline", color = 'rgb(255, 87, 34)', width = 3),
