...
rhoi_ba = ba_rumor_model(G_ba)
t_ba = seq(1,length(rhoi_ba)) # time steps


rhoi_er = ba_rumor_model(G_er)
t_er = seq(1,length(rhoi_er)) # time steps


plot(t_ba, rhoi_ba, xlab = "Time", 
     main="Rumor propagation model",
     ylab = "Fraction of infected nodes", 
     col = 'blue', lwd=2,ylim = c(0,0.2), xlim = c(0,Tmax), pch = 21,  bg = "blue", type="o")

lines(t_er, rhoi_er, col = 'green', lwd=2, pch = 21,  bg = "green", type="o")
legend("topright", legend=c("BA", "ER"),
