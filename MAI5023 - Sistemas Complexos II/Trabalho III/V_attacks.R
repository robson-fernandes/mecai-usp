...
#### FAILS ####
# BA network
df = 1 # remove only one node at each time step
...
step = 0
while(net.size > df){
  vk = degree(G) # calculate the node degree of each node
  nodes= which(vk %in% max(vk)) # select the node with the largest degree
...
plot(t,s/N,xlab = "Fraction of removed nodes", 
     ylab = "S/S0", type="l",lwd=2, col = 'blue', xlim = c(0,N/10), ylim = c(0,1))
...
while(net.size > df){# remove almost all nodes
  vk = degree(G2) # calculate the degree
  nodes= which(vk %in% max(vk)) # select the most connected nodes  
...
plot(x,y/N, xlab = "Fraction of removed nodes", 
     main = "Attacks - Random removal of nodes",
     ylab = "S/S0", type="l",lwd=2, col = 'red', xlim = c(0,N/10), ylim = c(0,1))
